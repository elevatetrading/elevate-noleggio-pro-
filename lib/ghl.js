...
export async function upsertContact(payload, expectedPhone) {
  ...
  if (contactId && tagsToAdd?.length) {
    // Rimuovi prima i tag esistenti per forzare il re-trigger del workflow GHL.
    // Se non erano presenti, la DELETE può fallire — non blocca il flusso.
    try {
      await axios.delete(
        `${BASE}/contacts/${contactId}/tags`,
        { headers: headers(), data: { tags: tagsToAdd } }
      );
